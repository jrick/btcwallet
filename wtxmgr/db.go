...
func upgradeStore(namespace walletdb.Namespace) error {
	...
	err := scopedUpdate(namespace, func(ns walletdb.Bucket) error {
		...
		_, err = ns.CreateBucketIfNotExists(bucketTxRecords)
		if err != nil {
			str := "failed to create tx records bucket"
			return storeError(ErrDatabase, str, err)
		}

		_, err = ns.CreateBucketIfNotExists(bucketCredits)
		if err != nil {
			str := "failed to create credits bucket"
			return storeError(ErrDatabase, str, err)
		}

		_, err = ns.CreateBucketIfNotExists(bucketDebits)
		if err != nil {
			str := "failed to create debits bucket"
			return storeError(ErrDatabase, str, err)
		}

		_, err = ns.CreateBucketIfNotExists(bucketUnspent)
		if err != nil {
			str := "failed to create unspent bucket"
			return storeError(ErrDatabase, str, err)
		}

		_, err = ns.CreateBucketIfNotExists(bucketUnmined)
		if err != nil {
			str := "failed to create unmined bucket"
			return storeError(ErrDatabase, str, err)
		}

		_, err = ns.CreateBucketIfNotExists(bucketUnminedCredits)
		if err != nil {
			str := "failed to create unmined credits bucket"
			return storeError(ErrDatabase, str, err)
		}

		_, err = ns.CreateBucketIfNotExists(bucketUnminedInputs)
		if err != nil {
